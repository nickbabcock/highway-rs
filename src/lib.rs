...
impl AvxHash {
    ...
    fn reset(&mut self) {
		let init0 = V4x64U::new(0x243f6a8885a308d3, 0x13198a2e03707344,
						   0xa4093822299f31d0, 0xdbe6d5d5fe4cce2f);
		let init1 = V4x64U::new(0x452821e638d01377, 0xbe5466cf34e90c6c,
						   0xc0acf169b5f18a8c, 0x3bd39e10cb0ef593);
    ...
    fn update_remainder(&mut self, bytes: &[u8]) {
        ...
        let shifted_right = V4x64U::from(unsafe { _mm256_srlv_epi32(self.v1.0, _mm256_sub_epi32(tip, size256)) });
        ...
        if size_mod32 & 16 != 0 {
            let packetL = unsafe {
                _mm_load_si128((&bytes[0] as *const u8) as *const __m128i)
            };
			let int_mask = unsafe { _mm_cmpgt_epi32(size, _mm_set_epi32(31, 27, 23, 19)) };
			let int_lanes = unsafe { _mm_maskload_epi32((&bytes[0] as *const u8).offset(16) as *const i32, int_mask) };
			...
			let packetL = unsafe { _mm_maskload_epi32(&bytes[0] as *const u8 as *const i32, int_mask) };
    ...
    fn zipper_merge(v: &V4x64U) -> V4x64U {
	 ...
	 fn update(&mut self, packet: V4x64U) {
		self.v1 += packet;
		self.v1 += self.mul0;
		self.mul0 ^= self.v1.mul_low32(&V4x64U::from(unsafe { _mm256_srli_epi64(self.v0.0, 32) }));
		self.v0 += self.mul1;
		self.mul1 ^= self.v0.mul_low32(&V4x64U::from(unsafe { _mm256_srli_epi64(self.v1.0, 32) }));
		self.v0 += AvxHash::zipper_merge(&self.v1);
		self.v1 += AvxHash::zipper_merge(&self.v0);
	}

    fn permute(v: &V4x64U) -> V4x64U {
        let indices = V4x64U::new(0x0000000200000003, 0x0000000000000001, 0x0000000600000007, 0x0000000400000005);
